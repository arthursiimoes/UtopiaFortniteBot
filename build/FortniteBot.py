...
def check_for_updates(current_version):
    ...
    if response.status_code == 200:
        ...
        if latest_version > current_version:
            messagebox.showinfo("Atualização Disponível", f"Uma nova versão {latest_version} está disponível. Por favor, atualize seu bot para que todos os erros possam ser corrigidos.")
...
def open_main_window():
    ...
    global main_window, countdown_label, elapsed_label, running
    current_version = "v0.2"  # Atualize para a versão atual
    check_for_updates(current_version)  # Verifica se há atualizações

    # Cria a janela principal
    main_window = tk.Tk()
    main_window.title("XP BOT v0.2")

    # Centraliza a janela principal
    center_window(main_window, 280, 400)
    ...
    start_button = ttk.Button(main_window, text="Iniciar", command=start_bot_thread)
...
def start_bot_thread():
    global running, start_time, countdown
    if not running:  # Verifica se o bot não está rodando
        running = True
        start_time = time.time()  # Marca o tempo de início
        countdown = interval  # Reseta o contador
        update_countdown()  # Atualiza o contador na interface
        threading.Thread(target=bot_loop, daemon=True).start()  # Inicia o loop do bot em uma thread separada
...
# Centraliza a janela de entrada
center_window(entry_window, 300, 150)

entry_window.configure(bg="#2C2F33")

# Verifica se a imagem do ícone existe antes de carregar
icon_path_entry = resource_path("imagens/u.png")
if os.path.exists(icon_path_entry):
    entry_window.iconphoto(False, ImageTk.PhotoImage(Image.open(icon_path_entry)))
else:
    print(f"Imagem de ícone '{icon_path_entry}' não encontrada.")
